...
def diff(source, destination):
    ...
    def sub_diff(field):
        source_items = {slugify(t[field]) for t in source}
        destination_items = {slugify(t[field]) for t in destination}
        return source_items.difference(destination_items)

    print(sub_diff("artist"))
    print("artists diff : {}".format(len(sub_diff("artist"))))
    print("albums diff : {}".format(len(sub_diff("album"))))
    print("titles diff : {}".format(len(sub_diff("title"))))
...
#
#
# @cli.command(help='''Print a filter''', 'filter')
# @click.pass_context
# @helpers.add_options(helpers.output_option)
# @click.argument('name')
...
#
#
# @cli.command(help='''Export music files to csv file''', 'csv')
