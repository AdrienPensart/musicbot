...
options = [
    ...
    click.option('--no-color', help='Disable colorized output', envvar='MB_NO_COLOR', default=DEFAULT_NO_COLOR, is_flag=True),
]
...
class Config(object):
    ...
    def set(self, debug=None, timings=None, dry=None, quiet=None, verbosity=None, no_color=None, log=None, **kwargs):
        ...
        debug = debug if debug is not None else lib.str2bool(os.getenv('MB_DEBUG', DEFAULT_DEBUG))
        ...
        self.no_color = no_color if no_color is not None else lib.str2bool(os.getenv('MB_NO_COLOR', DEFAULT_NO_COLOR))
        ...
        if not self.no_color:
            ...
            coloredlogs.install(level=self._verbosity, fmt=fmt)
            coloredlogs.install(level=self.verbosity, fmt=fmt)
        logger.debug(self)
    ...
    def verbosity(self, verbosity):
        ...
        if level is DEBUG:
            logger.debug('Loop debugging enabled')
            loop.set_debug(True)
        logger.debug('new verbosity: {}'.format(verbosity))

    def __repr__(self):
        return 'log:{} timings:{} debug:{} quiet:{} dry:{} verbosity:{} no_color:{}'.format(self.log, self.timings, self.debug, self.quiet, self.dry, self._verbosity, self.no_color)
